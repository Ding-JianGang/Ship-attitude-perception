...
class HRModule(nn.Module):
    ...
    def _make_fuse_layers(self):
        ...
        for i in range(num_out_branches):
            ...
            for j in range(num_branches):
                if j > i: # 当输入分支j大于输出分支i时，需要将第i个分支的输出进行上采样
                    fuse_layer.append(
                        nn.Sequential(
                            build_conv_layer(
                                self.conv_cfg,
                    ...
                    fuse_layer.append(None)   # 当输入分支j=输出分支i时，不做任何处理
                else:   # 当输入分支j＜输出分支i时，需要将第i个分支的输出进行下采样
                    conv_downsamples = []   # 初始化空列表
                    for k in range(i - j):
                        if k == i - j - 1:
                            conv_downsamples.append(
                                nn.Sequential(
                                    build_conv_layer(
                                        self.conv_cfg,
                                        in_channels[j],
                                        in_channels[i],  # 最后一个卷积层不仅要调整通道，还要进行下采样
                                        kernel_size=3,
                                        stride=2,
                                        padding=1,
                                        bias=False),
    ...
    def forward(self, x):
        ...
        for i in range(self.num_branches):
            x[i] = self.branches[i](x[i])  # 得到一个列表，该列表为每个分支通过Basicblock之后的输出

        x_fuse = []
        for i in range(len(self.fuse_layers)): # i代表第i个输出分支
            y = 0
            for j in range(self.num_branches):
                if i == j:
                    y += x[j]
                else:
                    y += self.fuse_layers[i][j](x[j]) # 利用fuse_layers对输出的结果进行上下采样及add融合
