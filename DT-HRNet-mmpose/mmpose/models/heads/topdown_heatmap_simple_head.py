...
class TopdownHeatmapSimpleHead(TopdownHeatmapBaseHead):
    ...
    def __init__(self,
                 in_channels,   # 输入通道数
                 out_channels,  # 输出通道数(每一个关键点的热力图)
                 num_deconv_layers=3,  # 转置卷积层数
                 num_deconv_filters=(256, 256, 256), # 转置卷积核的数量
                 num_deconv_kernels=(4, 4, 4), # 转置卷积卷积核尺寸
                 extra=None,
                 in_index=0, # 输入特征索引，默认为0
                 input_transform=None, # 为None代表只选择一个特征映射
                 align_corners=False,
                 loss_keypoint=None, # 关键点损失
                 train_cfg=None,
                 test_cfg=None):
        super().__init__()
        ...
        if identity_final_layer:
            ...
            layers.append(
                build_conv_layer(cfg=dict(type='Conv2d'),in_channels=conv_channels,out_channels=out_channels,kernel_size=kernel_size,stride=1,padding=padding))
